...
__global__ void calculateAverage(int *data, int *result, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N)
    {
        // Calculate the bin index for the data value
        int resultIdx = data[idx];
        atomicAdd(result, resultIdx); // Atomic update to avoid race conditions
    }
}
...
int main(int argc, char **argv)
{
    ...
    int *resultArrayHost, *randomNumbersDevice;
    int *deviceResult;
    int *hostResult;

    hostResult = (int *)calloc(1, sizeof(int));
    ...
    cudaMemset(deviceResult, 0.0, sizeof(int));

    // computeHistogram<<<blocksingrid, threadsinblock>>>(randomNumbersDevice, resultArrayDevice, N, A, B);
    calculateAverage<<<blocksingrid, threadsinblock>>>(randomNumbersDevice, deviceResult, N);
    ...
    double finalResult = (double)(*hostResult) / N;
